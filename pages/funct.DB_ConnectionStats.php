...
function storeConnectionStatsMeta(FileCacher $fc, $connStatsObj){
    $cacheKey = Config::DatabaseConnectionStatsMetaKey;
    
    $fc->put($cacheKey, $connStatsObj, false);
}

function getConnectionStatsMeta(FileCacher $fc){
    $cacheKey = Config::DatabaseConnectionStatsMetaKey;
...
function storeProcessList(FileCacher $fc, $connStatsObj){
    $cacheKey = Config::DatabaseConnectionStatsProclistKey;
    
    $fc->put($cacheKey, $connStatsObj, false);
}

function getStoredProcessList(FileCacher $fc){
    $cacheKey = Config::DatabaseConnectionStatsProclistKey;
...
function getStoredConnectionsObject(FileCacher $fc, $fForceNew = false){
...
function storeConnectionsObject(FileCacher $fc, $connStatsObj){
    $cacheKey = Config::DatabaseConnectionStatsKey;
...
function createCurrentTimeObject($fTimeStamp = false){
    ...
    foreach($LTR as $letter){
        $curDateObj->microTimeStamp = $microTimeStamp;
        $curDateObj->{$letter} = date($letter, $fTimeStamp);
    }
